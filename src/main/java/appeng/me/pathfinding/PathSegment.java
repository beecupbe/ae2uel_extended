...
public class PathSegment {
                ...
                if (!this.closed.contains(pi)) {
                    ...
                    if (flags.contains(GridFlags.REQUIRE_CHANNEL)) {
                        ...
                        if (!this.semiOpen.contains(pi)) {
                            ...
                            } else if (flags.contains(GridFlags.X64_CAPACITY)) {
                                worked = this.useX64Channel(pi);
                            } else if (flags.contains(GridFlags.X128_CAPACITY)) {
    ...
    private boolean useX64Channel(final IPathItem start) {
        IPathItem pi = start;
        while (pi != null) {
            if (!pi.canSupportX64Channels() || pi.getFlags().contains(GridFlags.CANNOT_CARRY_COMPRESSED)) {
                return false;
            }
    ...
    private boolean useX128Channel(final IPathItem start) {
        IPathItem pi = start;
        while (pi != null) {
            if (!pi.canSupportX128Channels() || pi.getFlags().contains(GridFlags.CANNOT_CARRY_COMPRESSED)) {
                return false;
            }
    ...
    private boolean useX256Channel(final IPathItem start) {
        IPathItem pi = start;
        while (pi != null) {
