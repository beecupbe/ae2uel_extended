...
public final class ItemMaterial extends AEBaseItem implements IStorageComponent, IUpgradeModule, ICellWorkbenchItem {
    ...
    public void addCheckedInformation(final ItemStack stack, final World world, final List<String> lines, final ITooltipFlag advancedTooltips) {
        ...
        if (u != null) {
            ...
            for (final Entry<ItemStack, Integer> j : u.getSupported().entrySet()) {
                ...
                if (j.getKey().getItem() instanceof final IItemGroup ig) {
    ...
    public EnumActionResult onItemUseFirst(final EntityPlayer player, final World world, final BlockPos pos, final EnumFacing side, final float hitX, final float hitY, final float hitZ, final EnumHand hand) {
        if (player.isSneaking()) {
            ...
            if (upgrades != null && !player.getHeldItem(hand).isEmpty() && player.getHeldItem(hand).getItem() instanceof final IUpgradeModule um) {
    ...
    public int getBytes(final ItemStack is) {
        switch (this.getTypeByStack(is)) {
            ...
            case CELL64MB_PART:
                return KILO_SCALAR * 65536;
            case CELL256MB_PART:
                return KILO_SCALAR * 262144;
            default:
        }
        return 0;
    }
    ...
    public boolean isStorageComponent(final ItemStack is) {
        switch (this.getTypeByStack(is)) {
            ...
            case CELL4K_PART:
            case CELL16K_PART:
            case CELL64K_PART, CELL1MB_PART, CELL4MB_PART, CELL16MB_PART, CELL64MB_PART, CELL256MB_PART:
                return true;
            default:
        }
        return false;
