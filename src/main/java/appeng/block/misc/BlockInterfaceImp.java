...
public class BlockInterfaceImp extends AEBaseTileBlock {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos) {
        ...
        TileInterfaceImp te = this.getTileEntity(world, pos);
    ...
    public boolean onActivated(final World w, final BlockPos pos, final EntityPlayer p, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        final TileInterfaceImp tg = this.getTileEntity(w, pos);
        if (tg != null) {
            if (Platform.isServer()) {
                Platform.openGUI(p, tg, AEPartLocation.fromFacing(side), GuiBridge.GUI_INTERFACEIMP);
            }
            return true;
        }
        return false;
    ...
    protected void customRotateBlock(final IOrientable rotatable, final EnumFacing axis) {
        if (rotatable instanceof TileInterfaceImp) {
            ((TileInterfaceImp) rotatable).setSide(axis);
        }
    }
}
