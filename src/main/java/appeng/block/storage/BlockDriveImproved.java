...
import appeng.tile.storage.TileDriveImproved;
...
public class BlockDriveImproved extends AEBaseTileBlock {
    ...
    public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
        TileDriveImproved te = this.getTileEntity(world, pos);
        IExtendedBlockState extState = (IExtendedBlockState) super.getExtendedState(state, world, pos);
        return extState.withProperty(SLOTS_STATE, te == null ? DriveSlotsState.createEmpty(30) : DriveSlotsState.fromChestOrDrive(te));
    }
    ...
    public boolean onActivated(final World w, final BlockPos pos, final EntityPlayer p, final EnumHand hand, final @Nullable ItemStack heldItem, final EnumFacing side, final float hitX, final float hitY, final float hitZ) {
        ...
        final TileDriveImproved tg = this.getTileEntity(w, pos);
        if (tg != null) {
            if (Platform.isServer()) {
                Platform.openGUI(p, tg, AEPartLocation.fromFacing(side), GuiBridge.GUI_DRIVE_IMPROVED);
            }
            return true;
        }
        return false;
