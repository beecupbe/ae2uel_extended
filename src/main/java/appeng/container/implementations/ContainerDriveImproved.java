...
public class ContainerDriveImproved extends AEBaseContainer {
    ...
    public ContainerDriveImproved(final InventoryPlayer ip, final TileDriveImproved drive) {
        super(ip, drive, null);

        for (int y = 0; y < 5; y++) {
            for (int x = 0; x < 6; x++) {
                this.addSlotToContainer(new SlotRestrictedInput(SlotRestrictedInput.PlacableItemType.STORAGE_CELLS, drive
                        .getInternalInventory(), x + y * 6, 36 + x * 18, 14 + y * 18, this.getInventoryPlayer()));
            }
        }
