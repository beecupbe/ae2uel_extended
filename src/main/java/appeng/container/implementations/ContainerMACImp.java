...
import appeng.container.slot.SlotMACPatternImp;
...
public class ContainerMACImp extends ContainerUpgradeable implements IProgressProvider {

    private static final int MAX_CRAFT_PROGRESS = 100;
    private final TileMolecularAssemblerImp tma;
    @GuiSync(4)
    public int craftProgress = 0;

    public ContainerMACImp(final InventoryPlayer ip, final TileMolecularAssemblerImp te) {
        super(ip, te);
        this.tma = te;
    }
    ...
    protected void setupConfig() {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                final SlotMACPatternImp s = new SlotMACPatternImp(this, mac, x + y * 3, offX + x * 18, offY + y * 18);
