...
public class DualityInterfacePer implements IGridTickable, IStorageMonitorable, IInventoryDestination, IAEAppEngInventory, IConfigManagerHost, ICraftingProvider, IUpgradeableHost {
    ...
    public DualityInterfacePer(final AENetworkProxy networkProxy, final IInterfaceHost ih) {
            ...
            try {
                ...
                if (!targetTE.getInterfaceDualityPer().sameGrid(this.gridProxy.getGrid())) {
                    ...
                    if (mon != null) {
                        ...
                        if (sm != null && Platform.canAccess(targetTE.getInterfaceDualityPer().gridProxy, this.mySource)) {
    ...
    public IStorageMonitorable getMonitorable(final IActionSource src, final IStorageMonitorable myInterface) {
        ...
        final DualityInterfacePer di = this;
    ...
    public String getTermName() {
        ...
        for (final EnumFacing direction : possibleDirections) {
            ...
            if (directedTile instanceof IInterfaceHost) {
                try {
                    if (((IInterfaceHost) directedTile).getInterfaceDualityPer().sameGrid(this.gridProxy.getGrid())) {
                        continue;
                    }
                } catch (final GridAccessException e) {
                    continue;
    ...
    private class InterfaceRequestContext implements Comparable<Integer> {
        ...
        public int compareTo(Integer o) {
            return Integer.compare(DualityInterfacePer.this.priority, o);
        }
    }
    ...
    private class InterfaceInventory extends MEMonitorIInventory {
        ...
        public InterfaceInventory(final DualityInterfacePer tileInterface) {
        ...
        public IAEItemStack extractItems(final IAEItemStack request, final Actionable type, final IActionSource src) {
            final Optional<InterfaceRequestContext> context = src.context(InterfaceRequestContext.class);
            final boolean hasLowerOrEqualPriority = context.map(c -> c.compareTo(DualityInterfacePer.this.priority) <= 0).orElse(false);
    ...
    private class Accessor implements IStorageMonitorableAccessor {
        ...
        public IStorageMonitorable getInventory(IActionSource src) {
            return DualityInterfacePer.this.getMonitorable(src, DualityInterfacePer.this);
        }
