...
import appeng.container.implementations.ContainerInterfaceTerminalPatt;
...
public class GuiInterfaceTerminalPatt extends AEBaseGui {
    ...
    private final PartInterfaceTerminalPatt partInterfaceTerminal;
    ...
    public GuiInterfaceTerminalPatt(final InventoryPlayer inventoryPlayer, final PartInterfaceTerminalPatt te) {
        super(new ContainerInterfaceTerminalPatt(inventoryPlayer, te));
    ...
    public void drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        for (int x = 0; x < rows && linesDraw < rows && currentScroll + x < this.lines.size(); x++) {
            ...
            if (lineObj instanceof final ClientDCInternalInv inv) {
                ...
                for (int row = 0; row < 8 && linesDraw < rows; ++row) {
                    for (int z = 0; z < 9; z++) {
    ...
    private ClientDCInternalInv getById(final long id, final long sortBy, final String string) {
        ...
        if (o == null) {
            this.byId.put(id, o = new ClientDCInternalInv(DualityInterfacePatt.NUMBER_OF_PATTERN_SLOTS, id, sortBy, string));
            this.refreshList = true;
        }
