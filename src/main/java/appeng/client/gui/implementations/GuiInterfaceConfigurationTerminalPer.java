...
public class GuiInterfaceConfigurationTerminalPer extends AEBaseGui implements IJEIGhostIngredients {
    ...
    public GuiInterfaceConfigurationTerminalPer(final InventoryPlayer inventoryPlayer, final PartInterfaceConfigurationTerminalPer te) {
        super(new ContainerInterfaceConfigurationTerminalPer(inventoryPlayer, te));
    ...
    public void drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        for (int x = 0; x < LINES_ON_PAGE && linesDraw < LINES_ON_PAGE && currentScroll + x < this.lines.size(); x++) {
            ...
            if (lineObj instanceof final ClientDCInternalInv inv) {
                ...
                for (int row = 0; row < 4 && linesDraw < LINES_ON_PAGE; ++row) {
                    for (int z = 0; z < 9; z++) {
    ...
    private ClientDCInternalInv getById(final long id, final long sortBy, final String string) {
        ...
        if (o == null) {
            this.byId.put(id, o = new ClientDCInternalInv(DualityInterfacePer.NUMBER_OF_CONFIG_SLOTS, id, sortBy, string, 512));
            this.refreshList = true;
        }
