...
public class PartInterfaceConfigurationTerminalAdv extends AbstractPartDisplay {
    ...
    public PartInterfaceConfigurationTerminalAdv(final ItemStack is) {
        super(is);
    }
    ...
    public boolean onPartActivate(final EntityPlayer player, final EnumHand hand, final Vec3d pos) {
        if (!super.onPartActivate(player, hand, pos)) {
            if (Platform.isServer()) {
                Platform.openGUI(player, this.getHost().getTile(), this.getSide(), GuiBridge.GUI_INTERFACE_CONFIGURATION_TERMINALADV);
            }
        }
        return true;
    }
