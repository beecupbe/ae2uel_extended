...
public class PartInterfaceTerminalAdv extends AbstractPartDisplay {
    ...
    public PartInterfaceTerminalAdv(final ItemStack is) {
        super(is);
    }
    ...
    public boolean onPartActivate(final EntityPlayer player, final EnumHand hand, final Vec3d pos) {
        if (!super.onPartActivate(player, hand, pos)) {
            if (Platform.isServer()) {
                Platform.openGUI(player, this.getHost().getTile(), this.getSide(), GuiBridge.GUI_INTERFACE_TERMINALADV);
            }
        }
        return true;
    }
