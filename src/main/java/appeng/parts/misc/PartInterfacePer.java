...
public class PartInterfacePer extends PartBasicState implements IGridTickable, IStorageMonitorable, IInventoryDestination, IInterfaceHost, IAEAppEngInventory, IPriorityHost {

    public static final ResourceLocation MODEL_BASE = new ResourceLocation(AppEng.MOD_ID, "part/interfaceper_base");
    ...
    private final DualityInterfaceImp duality2 = new DualityInterfaceImp(this.getProxy(), this);
    ...
    @Reflected
    public PartInterfacePer(final ItemStack is) {
        super(is);
    }
    ...
    public boolean onPartActivate(final EntityPlayer p, final EnumHand hand, final Vec3d pos) {
        if (Platform.isServer()) {
            Platform.openGUI(p, this.getTileEntity(), this.getSide(), GuiBridge.GUI_INTERFACEPER);
        }
        return true;
    }
    ...
    public ItemStack getItemStackRepresentation() {
        return AEApi.instance().definitions().parts().ifacePer().maybeStack(1).orElse(ItemStack.EMPTY);
    }

    @Override
    public GuiBridge getGuiBridge() {
        return GuiBridge.GUI_INTERFACEPER;
    }
