...
public class PartInterfaceImp extends PartBasicState implements IGridTickable, IStorageMonitorable, IInventoryDestination, IInterfaceHost, IAEAppEngInventory, IPriorityHost {

    public static final ResourceLocation MODEL_BASE = new ResourceLocation(AppEng.MOD_ID, "part/interfaceimp_base");
    ...
    @Reflected
    public PartInterfaceImp(final ItemStack is) {
        super(is);
    }
    ...
    public boolean onPartActivate(final EntityPlayer p, final EnumHand hand, final Vec3d pos) {
        if (Platform.isServer()) {
            Platform.openGUI(p, this.getTileEntity(), this.getSide(), GuiBridge.GUI_INTERFACEIMP);
        }
        return true;
    }
    ...
    public ItemStack getItemStackRepresentation() {
        return AEApi.instance().definitions().parts().ifaceImp().maybeStack(1).orElse(ItemStack.EMPTY);
    }
